...
#listWidgetで扱うノードタイプが第一引数となる。
class listWidgetWithButton(QWidget):
    itemList = []
    valueChanged = Signal(float)
    labelType = ""
    #buttonSignal = Signal(str)

    def __init__(self,nodeType, *args, **kwargs):
        super(listWidgetWithButton, self).__init__(*args, **kwargs)
        self.nodeType = nodeType
        self.settingUI()
        self.setObjectName("mm_listWidgetWithButton")
    ...
    def settingUI(self):
        ...
        #addボタン
        self.buttonA = QPushButton("add")
        self.buttonA.setObjectName("mm_button_add")
        self.buttonA.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Preferred)
        lay_right.addWidget(self.buttonA)

        #deleteボタン
        self.buttonB = QPushButton("delete")
        self.buttonB.setObjectName("mm_button_delete")
        self.buttonB.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Preferred)
        lay_right.addWidget(self.buttonB)

        #レイアウト追加
        lay.addLayout(lay_right)
        self.setLayout(lay)

    #addボタン・deleteボタンでそれぞれ異なったシグナルをemit
